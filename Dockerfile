...
LABEL org.opencontainers.image.source=https://github.com/volcie/mtasa-docker \
...
RUN echo "deb http://deb.debian.org/debian bullseye main" > /etc/apt/sources.list.d/bullseye.list \
	...
	&& apt-get update \
	&& apt-get install -y --no-install-recommends ca-certificates libncursesw6 wget netcat-openbsd gosu unzip \
...
# ncurses needs TERM to properly render the interactive console
ENV TERM=xterm
...
# Entrypoint runs as root to fix volume permissions, then drops to mtasa
USER root
